...
if $ipCMD2 l show v-dn42 &>/dev/null || $ipCMD1 l show v-agents &>/dev/null; then
...
ip l add v-dn42 netns $pid2 type veth peer v-agents netns $pid1
if [ $? -ne 0 ]; then
...
$ipCMD1 l set v-agents vrf vrf42
$ipCMD1 l set v-agents up
$ipCMD1 a flush scope link dev v-agents
$ipCMD1 a add fe80::1/64 dev v-agents
$ipCMD1 route replace $DN42_ULA_ADDR/128 via fe80::2 dev v-agents vrf vrf42
